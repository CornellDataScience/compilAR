...
constexpr int kStragglerRank  = 7;
...
ncclGroupStart(); 
if (myRank == 0) {
                    CHECK_NCCL(ncclSend(d_buffer + 0 * chunkSize,           chunkSize, ncclFloat, 7, comm, stream));
...
ncclGroupEnd();
if (myRank == 0 || myRank == 7) reduce_add<<<nb, kReduceThreads, 0, stream>>>(d_buffer + 0*chunkSize, d_tempbuf, chunkSize); 
ncclGroupStart(); 
if (myRank == 1) {
...
if (myRank == 0) { CHECK_NCCL(ncclSend(d_buffer + 0 * chunkSize, chunkSize, ncclFloat, 3, comm, stream)); } 
if (myRank == 3) { CHECK_NCCL(ncclRecv(d_buffer + 0 * chunkSize, chunkSize, ncclFloat, 0, comm, stream)); } 
ncclGroupEnd();
if (myRank == 1 || myRank == 7) reduce_add<<<nb, kReduceThreads, 0, stream>>>(d_buffer + 1*chunkSize, d_tempbuf, chunkSize); 
ncclGroupStart(); 
if (myRank == 2) {
...
if (myRank == 1) { CHECK_NCCL(ncclSend(d_buffer + 1 * chunkSize, chunkSize, ncclFloat, 4, comm, stream)); } 
if (myRank == 4) { CHECK_NCCL(ncclRecv(d_buffer + 1 * chunkSize, chunkSize, ncclFloat, 1, comm, stream)); } 
if (myRank == 0) { CHECK_NCCL(ncclSend(d_buffer + 0 * chunkSize, chunkSize, ncclFloat, 5, comm, stream)); } 
if (myRank == 5) { CHECK_NCCL(ncclRecv(d_buffer + 0 * chunkSize, chunkSize, ncclFloat, 0, comm, stream)); } 
if (myRank == 3) { CHECK_NCCL(ncclSend(d_buffer + 0 * chunkSize, chunkSize, ncclFloat, 6, comm, stream)); } 
if (myRank == 6) { CHECK_NCCL(ncclRecv(d_buffer + 0 * chunkSize, chunkSize, ncclFloat, 3, comm, stream)); } 
ncclGroupEnd();
if (myRank == 2 || myRank == 7) reduce_add<<<nb, kReduceThreads, 0, stream>>>(d_buffer + 2*chunkSize, d_tempbuf, chunkSize); 
ncclGroupStart(); 
if (myRank == 3) {
...
if (myRank == 0) {
                    CHECK_NCCL(ncclRecv(d_buffer + 1*chunkSize, chunkSize, ncclFloat, 4, comm, stream));
                    CHECK_NCCL(ncclSend(d_buffer + 0*chunkSize, chunkSize, ncclFloat, 4, comm, stream));
                } 
if (myRank == 5) {
...
if (myRank == 1) {
                    CHECK_NCCL(ncclRecv(d_buffer + 0*chunkSize, chunkSize, ncclFloat, 5, comm, stream));
                    CHECK_NCCL(ncclSend(d_buffer + 1*chunkSize, chunkSize, ncclFloat, 5, comm, stream));
                } 
if (myRank == 6) {
...
if (myRank == 2) {
                    CHECK_NCCL(ncclRecv(d_buffer + 0*chunkSize, chunkSize, ncclFloat, 6, comm, stream));
                    CHECK_NCCL(ncclSend(d_buffer + 2*chunkSize, chunkSize, ncclFloat, 6, comm, stream));
                } 
ncclGroupEnd();
if (myRank == 3 || myRank == 7) reduce_add<<<nb, kReduceThreads, 0, stream>>>(d_buffer + 3*chunkSize, d_tempbuf, chunkSize); 
ncclGroupStart(); 
if (myRank == 4) {
...
ncclGroupEnd();
if (myRank == 4 || myRank == 7) reduce_add<<<nb, kReduceThreads, 0, stream>>>(d_buffer + 4*chunkSize, d_tempbuf, chunkSize); 
ncclGroupStart(); 
if (myRank == 5) {
...
ncclGroupEnd();
if (myRank == 5 || myRank == 7) reduce_add<<<nb, kReduceThreads, 0, stream>>>(d_buffer + 5*chunkSize, d_tempbuf, chunkSize); 
ncclGroupStart(); 
if (myRank == 6) {
...
if (myRank == 0) {
                    ...
                    CHECK_NCCL(ncclRecv(d_buffer + 4*chunkSize, chunkSize, ncclFloat, 2, comm, stream));
                } 
if (myRank == 2) {
                    CHECK_NCCL(ncclRecv(d_buffer + 3*chunkSize, chunkSize, ncclFloat, 0, comm, stream));
...
ncclGroupEnd();
if (myRank == 6 || myRank == 7) reduce_add<<<nb, kReduceThreads, 0, stream>>>(d_buffer + 6*chunkSize, d_tempbuf, chunkSize); 
ncclGroupStart(); 
if (myRank == 7) { CHECK_NCCL(ncclSend(d_buffer + 6 * chunkSize, chunkSize, ncclFloat, 4, comm, stream)); } 
if (myRank == 4) { CHECK_NCCL(ncclRecv(d_buffer + 6 * chunkSize, chunkSize, ncclFloat, 7, comm, stream)); } 
if (myRank == 0) {
                    CHECK_NCCL(ncclSend(d_buffer + 4*chunkSize, chunkSize, ncclFloat, 3, comm, stream));
...
if (myRank == 5) {
                    CHECK_NCCL(ncclRecv(d_buffer + 4*chunkSize, chunkSize, ncclFloat, 1, comm, stream));
...
if (myRank == 6) {
                    CHECK_NCCL(ncclRecv(d_buffer + 4*chunkSize, chunkSize, ncclFloat, 2, comm, stream));
                    CHECK_NCCL(ncclSend(d_buffer + 6*chunkSize, chunkSize, ncclFloat, 2, comm, stream));
                } 
ncclGroupEnd();
ncclGroupStart(); 
if (myRank == 7) { CHECK_NCCL(ncclSend(d_buffer + 6 * chunkSize, chunkSize, ncclFloat, 5, comm, stream)); } 
if (myRank == 5) { CHECK_NCCL(ncclRecv(d_buffer + 6 * chunkSize, chunkSize, ncclFloat, 7, comm, stream)); } 
if (myRank == 0) {
...
if (myRank == 1) {
                    ...
                    CHECK_NCCL(ncclRecv(d_buffer + 6*chunkSize, chunkSize, ncclFloat, 4, comm, stream));
                } 
if (myRank == 4) {
                    CHECK_NCCL(ncclRecv(d_buffer + 5*chunkSize, chunkSize, ncclFloat, 1, comm, stream));
