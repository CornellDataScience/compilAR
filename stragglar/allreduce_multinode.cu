    ...
    cudaEvent_t stop, int chunkSize)
{
  ...
  if (myRank == 0 || myRank == 3)
    reduce_add<<<nb, kReduceThreads, 0, stream>>>(d_buffer, d_tempbuf, chunkSize);

  // step 2: ranks 1 and 3 swap chunk 1; rank 0 pushes chunk 0 to 2
  ncclGroupStart();
  if (myRank == 3) {
    CHECK_NCCL(ncclSend(d_buffer + chunkSize, chunkSize, ncclFloat, 1, comm, stream));
    CHECK_NCCL(ncclRecv(d_tempbuf, chunkSize, ncclFloat, 1, comm, stream));
  } else if (myRank == 1) {
    CHECK_NCCL(ncclRecv(d_tempbuf, chunkSize, ncclFloat, 3, comm, stream));
    CHECK_NCCL(ncclSend(d_buffer + chunkSize, chunkSize, ncclFloat, 3, comm, stream));
  } else if (myRank == 0) {
    CHECK_NCCL(ncclSend(d_buffer, chunkSize, ncclFloat, 2, comm, stream));
  } else if (myRank == 2) {
    CHECK_NCCL(ncclRecv(d_buffer, chunkSize, ncclFloat, 0, comm, stream));
  }
  ncclGroupEnd();

  if (myRank == 1 || myRank == 3)
    reduce_add<<<nb, kReduceThreads, 0, stream>>>(d_buffer + chunkSize, d_tempbuf, chunkSize);

  // step 3: ranks 2 and 3 swap chunk 2; ranks 0 and 1 swap chunks
  ncclGroupStart();
  if (myRank == 3) {
    ...
    CHECK_NCCL(ncclSend(d_buffer,               chunkSize, ncclFloat, 1, comm, stream));
    CHECK_NCCL(ncclRecv(d_buffer + chunkSize,   chunkSize, ncclFloat, 1, comm, stream));
  } else if (myRank == 1) {
    CHECK_NCCL(ncclRecv(d_buffer,               chunkSize, ncclFloat, 0, comm, stream));
    CHECK_NCCL(ncclSend(d_buffer + chunkSize,   chunkSize, ncclFloat, 0, comm, stream));
  }
  ncclGroupEnd();

  if (myRank == 2 || myRank == 3)
    reduce_add<<<nb, kReduceThreads, 0, stream>>>(d_buffer + 2*chunkSize, d_tempbuf, chunkSize);

  // step 4: broadcast final reduced chunks to remaining ranks
  ncclGroupStart();
  if (myRank == 3) {
    CHECK_NCCL(ncclSend(d_buffer + 2*chunkSize, chunkSize, ncclFloat, 1, comm, stream));
  } else if (myRank == 1) {
    CHECK_NCCL(ncclRecv(d_buffer + 2*chunkSize, chunkSize, ncclFloat, 3, comm, stream));
  } else if (myRank == 0) {
    CHECK_NCCL(ncclSend(d_buffer + chunkSize, chunkSize, ncclFloat, 2, comm, stream));
    CHECK_NCCL(ncclRecv(d_buffer + 2*chunkSize, chunkSize, ncclFloat, 2, comm, stream));
  } else if (myRank == 2) {
    CHECK_NCCL(ncclRecv(d_buffer + chunkSize, chunkSize, ncclFloat, 0, comm, stream));
    CHECK_NCCL(ncclSend(d_buffer + 2*chunkSize, chunkSize, ncclFloat, 0, comm, stream));
  }
  ncclGroupEnd();

  // Rank 0 records stop after its stream finishes step 4
  if (myRank == 0) {
    cudaStreamSynchronize(stream);
